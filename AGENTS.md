...
### Syncing API types to web

When API types/models change, keep frontend types in sync by running the generated sync script. This ensures `apps/web/src/types/api.ts` always reflects the canonical API types.

1. Regenerate frontend types from the API (run from repo root):

```bash
pnpm run sync:api-types
```

2. The script `scripts/sync-api-types.mjs` scans `apps/api/src` for exported `interface`, `type`, and `enum` declarations and writes/overwrites `apps/web/src/types/api.ts`.

3. After running the script, it's recommended to run a quick typecheck and tests for the web app:

```bash
pnpm --filter @shop-wise/web run typecheck
pnpm --filter @shop-wise/web run test
```

4. To automate this, add the `pnpm run sync:api-types` command to your CI pipeline or a pre-commit hook so frontend types are always updated when API types change.

Note for CI: This repository includes a GitHub Actions workflow at `.github/workflows/sync-api-types.yml` which runs `pnpm run sync:api-types` and fails the job if `apps/web/src/types/api.ts` is modified by the script. Keep the workflow in sync if you move or rename the generator script.
